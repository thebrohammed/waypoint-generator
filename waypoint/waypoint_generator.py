...
def interpolate_disc_pwm(swath_width, swath_points):
    """Interpolate disc PWM value based on required swath width."""
    swath_widths = sorted(swath_points.keys())
    if not swath_widths:
        return 1500  # Default PWM if no swath points
    for i in range(len(swath_widths) - 1):
        swath1, swath2 = swath_widths[i], swath_widths[i + 1]
        pwm1, pwm2 = swath_points[swath1], swath_points[swath2]
        if swath1 <= swath_width <= swath2:
            # Linear interpolation
            slope = (pwm2 - pwm1) / (swath2 - swath1)
            return pwm1 + slope * (swath_width - swath1)
    # Clamp to min/max PWM if swath_width is outside range
    min_swath, max_swath = min(swath_widths), max(swath_widths)
    if swath_width < swath_points[min_swath]:
        return swath_points[min_swath]
    return swath_points[max_swath]
...
def load_swath_csv(swath_csv):
    """Load swath width data from CSV with PWM and Swath Width(m) columns."""
    try:
        df = pd.read_csv(swath_csv)
        if 'PWM' not in df.columns or 'Swath Width(m)' not in df.columns:
            messagebox.showerror("Error", "CSV must contain 'PWM' and 'Swath Width(m)' columns")
            return None
        swath_points = {float(row['Swath Width(m)']): float(row['PWM']) for _, row in df.iterrows()}
        return swath_points
    except Exception as e:
        messagebox.showerror("Error", f"Failed to read swath CSV: {e}")
        return None


def generate_waypoints(input_csv, altitude, speed, cal_points, valve_servo_channel, disc_servo_channel, swath_width, swath_points, include_takeoff):
    ...
    # Initialize servos to 1000 PWM after takeoff (or home if no takeoff)
    ...
    # Set disc speed PWM to target
    disc_pwm = interpolate_disc_pwm(swath_width, swath_points)
...
def browse_swath_file():
    """Open file dialog to select swath width CSV file."""
    filepath = filedialog.askopenfilename(filetypes=[("CSV Files", "*.csv")])
    if filepath:
        swath_file_entry.delete(0, tk.END)
        swath_file_entry.insert(0, filepath)
...
def visualize_waypoints(save_path=None):
    ...
    try:
        ...
        swath_path = swath_file_entry.get()
        cal_points = load_calibration_csv(cal_path) if cal_path else None
        swath_points = load_swath_csv(swath_path) if swath_path else None

        speed = float(speed_entry.get())
        swath_width = float(swath_width_entry.get())
        altitude = float(alt_entry.get())  # Not used here but for consistency

        if cal_points is None or swath_points is None:
            messagebox.showwarning("Warning", "Calibration or swath files not loaded. PWMs will not be shown.")
            disc_pwm = None
        else:
            disc_pwm = interpolate_disc_pwm(swath_width, swath_points)
        ...
        for i, row in df.iterrows():
            ...
            if coord:
                ...
                if cal_points:
                    current_coord = (lat, lon)  # Note: geodesic uses (lat, lon)
                    time_seconds = 1.0
                    if previous_coord:
                        distance_m = geodesic(previous_coord, current_coord).meters
                        time_seconds = distance_m / speed if speed > 0 else 1.0
        ...
        if lons and lats and grams:
            ...
            cmap = plt.get_cmap('RdBu')  # Red to blue gradient
...
def visualize_swath():
    """Visualize swath width data from CSV as a line plot."""
    try:
        swath_csv = swath_file_entry.get()
        if not swath_csv:
            messagebox.showerror("Error", "Please select a swath width CSV file")
            return
        df = pd.read_csv(swath_csv)
        if 'PWM' not in df.columns or 'Swath Width(m)' not in df.columns:
            messagebox.showerror("Error", "CSV must contain 'PWM' and 'Swath Width(m)' columns")
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        swath_widths = df['Swath Width(m)']
        pwms = df['PWM']
        ax.plot(swath_widths, pwms, 'o-', color='teal', linewidth=2, markersize=8, markeredgecolor='black')
        ax.set_title("Disc PWM vs. Swath Width", fontsize=16, weight='bold', color='navy')
        ax.set_xlabel("Swath Width (m)", fontsize=12)
        ax.set_ylabel("Disc PWM (us)", fontsize=12)
        ax.grid(True, linestyle='--', alpha=0.7)

        # Set axis limits with padding
        swath_range = max(swath_widths) - min(swath_widths)
        pwm_range = max(pwms) - min(pwms)
        ax.set_xlim(min(swath_widths) - 0.05 * swath_range, max(swath_widths) + 0.05 * swath_range)
        ax.set_ylim(min(pwms) - 0.05 * pwm_range, max(pwms) + 0.05 * pwm_range)
...
def run():
    ...
    try:
        ...
        swath_points = load_swath_csv(swath_path)
        if cal_points is None or swath_points is None:
            return
    except ValueError:
    ...
    if output_path:
        waypoints = generate_waypoints(input_path, altitude, speed, cal_points, valve_servo_channel, disc_servo_channel, swath_width, swath_points, include_takeoff)
...
# Swath Width CSV File Input
ttk.Label(frame, text="Swath Width CSV:", font=('Helvetica', 12, 'bold'), foreground='#1E3A8A').grid(row=3, column=0, sticky='e', pady=10)
swath_file_entry = ttk.Entry(frame, width=40, font=('Helvetica', 11))
swath_file_entry.grid(row=3, column=1, columnspan=2, padx=10, pady=10, sticky='ew')
ttk.Button(frame, text="Browse", bootstyle="light", command=browse_swath_file).grid(row=3, column=3, padx=10, pady=10)
ttk.Button(frame, text="📈 Plot Swath", bootstyle="light", command=visualize_swath).grid(row=3, column=4, padx=10, pady=10)

# Altitude
ttk.Label(frame, text="Altitude (m):", font=('Helvetica', 12), foreground='#1E3A8A').grid(row=4, column=0, sticky='e', pady=10)
alt_entry = ttk.Entry(frame, width=10, font=('Helvetica', 11))
alt_entry.insert(0, "10")
alt_entry.grid(row=4, column=1, sticky='w', padx=10)

# Speed
ttk.Label(frame, text="Speed (m/s):", font=('Helvetica', 12), foreground='#1E3A8A').grid(row=5, column=0, sticky='e', pady=10)
speed_entry = ttk.Entry(frame, width=10, font=('Helvetica', 11))
speed_entry.insert(0, "3")
speed_entry.grid(row=5, column=1, sticky='w', padx=10)

# Valve Servo Channel
ttk.Label(frame, text="Valve Servo Channel (1–16):", font=('Helvetica', 12), foreground='#1E3A8A').grid(row=6, column=0, sticky='e', pady=10)
...
valve_servo_dropdown.grid(row=6, column=1, sticky='w', padx=10)

# Disc Servo Channel
ttk.Label(frame, text="Disc Servo Channel (1–16):", font=('Helvetica', 12), foreground='#1E3A8A').grid(row=7, column=0, sticky='e', pady=10)
...
disc_servo_dropdown.grid(row=7, column=1, sticky='w', padx=10)

# Required Swath Width
ttk.Label(frame, text="Required Swath Width (m):", font=('Helvetica', 12), foreground='#1E3A8A').grid(row=8, column=0, sticky='e', pady=10)
swath_width_entry = ttk.Entry(frame, width=10, font=('Helvetica', 11))
swath_width_entry.insert(0, "5")
swath_width_entry.grid(row=8, column=1, sticky='w', padx=10)

# Include Takeoff
ttk.Label(frame, text="Include Takeoff Command:", font=('Helvetica', 12), foreground='#1E3A8A').grid(row=9, column=0, sticky='e', pady=10)
include_takeoff_var = tk.BooleanVar(value=True)
ttk.Checkbutton(frame, variable=include_takeoff_var, bootstyle="round-toggle").grid(row=9, column=1, sticky='w', padx=10)
